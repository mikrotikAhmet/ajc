		...
		if($this->input->post()){
			...
			if($this->error === '' && $this->input->post('step') && $this->input->post('step') == 3){
				...
				if(mysqli_multi_query($this->db->conn_id, $database)){
					...
					$data['password'] = $hasher->HashPassword($this->input->post('admin_passwordr'));
					$data['email'] = $this->input->post('admin_email');
					$data['datecreated'] = date('Y-m-d H:i:s');
					$data['admin'] = 1;
					$data['active'] = 1;

					// Set invoice year to current year
					update_option('invoice_year',date('Y'));
					update_option('estimate_year',date('Y'));
